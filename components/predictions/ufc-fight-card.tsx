...
export function UFCFightCard({ fight, fightNumber }: UFCFightCardProps) {
  ...
  const getImage = (primary?: string | null, fallback?: string | null) =>
    primary || fallback || "/placeholder.svg"
    ...
    <div className="card-elevated overflow-hidden">
      ...
      <div className="p-4">
        <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center mb-4">
          {/* Fighter A */}
          <div className={`flex items-center gap-3 ${isPredictedWinner(fight.fighter_a_name) ? "" : "flex-row-reverse"}`}>
            <div className="relative flex-shrink-0">
              <Image
                src={getImage(fight.fighter_a_image_url, fight.fighter_a_profile?.image_url)}
          ...
          <div className={`flex items-center gap-3 ${isPredictedWinner(fight.fighter_a_name) ? "flex-row-reverse" : ""}`}>
            <div className="relative flex-shrink-0">
              <Image
                src={getImage(fight.fighter_b_image_url, fight.fighter_b_profile?.image_url)}
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          <div className="bg-success/20 border border-success/50 rounded-lg p-3">
            ...
            <div className="text-sm font-bold text-success truncate">
              {predictedWinner ? predictedWinner.split(" ")[0] : "Pending"}
            </div>
          </div>
          ...
          <div className="card-elevated p-3">
            ...
            <div className={`text-lg font-mono font-bold ${hasConfidence ? getConfidenceColor(confidence) : "text-muted-foreground"}`}>
              {confidenceLabel}
            </div>
            <div className="mt-2 h-1.5 bg-secondary rounded-full overflow-hidden">
              <div
                className={`h-full transition-all ${hasConfidence ? getConfidenceBg(confidence) : "bg-secondary"}`}
          ...
          <div className="card-elevated p-3">
            ...
            <div className={`text-lg font-bold ${hasEdge ? (edgeVsMarket >= 0 ? "text-success" : "text-danger") : "text-muted-foreground"}`}>
              {hasEdge ? (
                <>
                  {edgeVsMarket >= 0 ? "+" : ""}
              ...
              {keyEdges.length ? (
                <ul className="space-y-1">
                  {keyEdges.map((edge, index) => (
                    <li key={`${fight.id}-edge-${index}`} className="text-sm text-muted-foreground flex items-start gap-2">
